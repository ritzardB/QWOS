...
File:
    test_sqlalchemy_employee_work_agreement_repository.py

Description:
    Unit tests for SQLAlchemyEmployeeWorkAgreementRepository.
...
from qwos.infrastructure.repositories.attendance.sqlalchemy_employee_work_agreement_repository import (
...
def make_repository():
    """
    Build repository with a fake session.
    """

    session = FakeSession()

    repository = SQLAlchemyEmployeeWorkAgreementRepository(
        session=session,
    )
...
def test_get_by_id_for_tenant_returns_agreement() -> None:
    repository, session = make_repository()

    expected = SimpleNamespace(
        id="01AGREEMENT00000000000000001",
    ...
    result = repository.get_by_id_for_tenant(
        tenant_id="01TENANT00000000000000000001",
        agreement_id="01AGREEMENT00000000000000001",
    )
...
def test_get_effective_for_employee_returns_agreement() -> None:
    repository, session = make_repository()
...
def test_list_by_employee_returns_agreements() -> None:
    repository, session = make_repository()
    ...
    expected = [
        ...
        SimpleNamespace(
            id="01AGREEMENT00000000000000002",
            employee_id="01EMPLOYEE00000000000000001",
        ),
    ]
...
def test_list_by_employee_returns_empty_list_when_not_found() -> None:
    repository, session = make_repository()
...
def test_get_active_by_employee_returns_agreement() -> None:
    repository, session = make_repository()
